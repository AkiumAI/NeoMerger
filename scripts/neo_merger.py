...
def ties_merge(tensors: list, alphas: list, density: float = 0.5) -> torch.Tensor:
    ...
    # Trim — keep only top-density weights by magnitude
    trimmed = []
    for tv in task_vectors:
        threshold = _quantile_safe(tv, 1.0 - density)
        trimmed.append(tv * (tv.abs() >= threshold))

    # Elect sign — majority vote on sign per element
    sign_sum = sum(t.sign() for t in trimmed)
    elected  = sign_sum.sign()

    # Merge — sum only weights that agree with elected sign
...
def build_block_sliders(default=0.5):
    ...
    # Description panels — hidden by default
    desc_panels = {}
        ...
        with gr.Tabs():
            ...
            with gr.Tab("🔀  Merge"):
                ...
                def bm_run(method, alpha, use_blocks, mode, ma, mb, mc,
                    ...
                    try:
                        merge_models_method(pa, pb, pc, bw, float(alpha),
                ...
                def bm_load_p(name):
                    ...
                    if not d: return ["❌  Not found."] + [gr.update()] * (len(bm_all) + 4)
                    ...
                    return ([f"✅  Loaded **{name}**.",
                             ...
                             gr.update(value=d.get("alpha", 0.5)),
                             gr.update(value=d.get("use_blocks", False)),
            ...
            with gr.Tab("🔍  Inspect"):
                ...
                def do_clear_metadata(name):
                    ...
                    # Write to a temp file first — if anything goes wrong
                    # the original is untouched until the very last step
                    ...
                    try:
                        ...
                        # Read all tensors into RAM
                        tensors = {}
                        with safe_open(path, framework="pt", device="cpu") as sf:
                            for key in sf.keys():
                                tensors[key] = sf.get_tensor(key)

                        # Write to temp file (no metadata)
                        save_file(tensors, tmp_path)
                        del tensors
                        torch.cuda.empty_cache()

                        # Only now replace the original — atomic on most OS
